...
A single workflow (`.github/workflows/cve-scan.yml`) with five jobs, in order: `scan`, `conformance`, `verify`, `collect`, and `rebuild`.
...
**Job 2: `conformance`** (`needs: scan`, `timeout-minutes: 10`)

Runs the build-conformance drift check exactly once, before any build, and gates every build leg. `build_conformance.py` fetches valkey-container's `ci.yml` and fails the job loudly if the build contract we mirror has drifted: the platform list, no `context:` override, no `build-args`, no `target:`, the `<dir>/Dockerfile` file shape, `provenance: false`, and that the `docker/*` action SHAs we pin still match theirs. It carries the same guards as `verify` (canonical `main`, candidates present, not a dry run), so a transient fetch failure never fails a routine scan. This check exists because we replicate their build rather than calling their workflow (see the tradeoffs), so a change on their side must break loudly, not silently invalidate our proof.

**Job 3: `verify`** (`needs: [scan, conformance]`, `timeout-minutes: 180`, one matrix leg per candidate)
...
1. **Build the candidate ourselves**: checks out `valkey-io/valkey-container` at `mainline` and builds ONE platform per leg with `docker/build-push-action`, mirroring their build exactly: `file: ./<line>/<variant>/Dockerfile` with no `context:` override (so the context is the repo root and the root `docker-entrypoint.sh` is what ships), `provenance: false`, `push: false`, `load: true`, tagged locally as `cve-candidate:<line>-<variant>-<platform-slug>`. There are no registry credentials anywhere in this job: the candidate never leaves the runner.
...
**Job 4: `collect`** (`needs: [scan, verify]`, `timeout-minutes: 10`)
...
**Job 5: `rebuild`** (`needs: [scan, collect]`, `timeout-minutes: 240`, conditional, automatic)
...
- **The drift check exists because we mirror, not call.** We replicate valkey-container's build settings (`file`, context, provenance) instead of invoking their workflow. `build_conformance.py` runs as a separate `conformance` job before any build leg and fails loudly if their build contract drifts, so the mirror can never silently go stale.
...
- **CVE scan: no AI in the pipeline** - the entire scan-verify-dispatch path is deterministic code (scanner, candidate build, `verify_candidate`, `build_conformance`, `gh workflow run`). No AI layer participates in any decision or dispatch step.
