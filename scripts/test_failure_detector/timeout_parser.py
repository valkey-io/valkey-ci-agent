...
#
# Anchored to the start of the runner's own line, allowing the Actions log
# timestamp and the summary's "*** " prefix ahead of the tag. A test that prints or asserts on text containing
# "[TIMEOUT]: <name> in tests/x.tcl" would otherwise be read as a timeout the
# runner never reported, filing an issue for a test that passed.
...
def jobs_needing_log_scan(
    ...
    These are candidates for timeout recovery. Captured assertions or other
    failures say nothing about whether the job also timed out, and a runner that
    does not record timeouts leaves a timed-out job with no timeout entry at
    all. Only an explicit timeout-type entry makes log scanning redundant for
    that job.
...
def find_job_log(
    ...
    A caller can hold either spelling of a matrix job: the API's
    ``base (value)`` or the artifact's ``base-value``. Both are tried, so a
    lookup keyed on the artifact's spelling still finds the log.
    """
    ...
    # Last resort: compare normalized forms, which folds the API's
    # "base (value)" and the artifact's "base-value" onto one key.
    normalized = normalize_job_name(job_name)
    for filename, content in logs.items():
        if "/" in filename or not filename.endswith(".txt"):
            continue
        name_part = re.sub(r"^\d+_", "", filename).removesuffix(".txt")
        if normalize_job_name(name_part) == normalized:
            return content
